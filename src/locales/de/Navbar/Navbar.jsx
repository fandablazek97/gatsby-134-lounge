  ...
  return (
    <nav
      ...
      className={`fixed z-100 w-screen transform-gpu transition-[background,transform,height,box-shadow] duration-400 ease-in-out ${
        isNavScrolled
          ? "h-20 bg-gray-900 shadow-soft-xl dark:shadow-none"
          : "h-20 bg-gradient-to-b from-gray-900 md:h-32"
    ...
    >
        ...
        <ul className="hidden list-none space-x-7 xl:flex 2xl:space-x-12">
          {mainRoutes.map((item) => (
            <li key={item.label}>
              <AnchorLink
                to={item.path}
                className="ui-link data-link-2 text-xs uppercase tracking-wider text-rich"
              >
                {item.label}
              </AnchorLink>
            </li>
      ...
      <div
        className={`fixed inset-0 block h-screen w-screen transform-gpu bg-gray-900 transition-visibility ease-out ${
          isMenuOpen
            ...
            {mainRoutes.map((item) => (
              <li key={item.label}>
                <AnchorLink
                  to={item.path}
                  onClick={closeMenu}
                  className="ui-link data-link-2 text-xl font-semibold uppercase text-rich"
                >
                  {item.label}
                </AnchorLink>
              </li>
